    ...
    async showRatingsList() {
        ...
        try {
            ...
            container.innerHTML = players.map(player => {
                ...
                return `
                    <div class="player-card card p-4 text-center cursor-pointer" onclick="ratePlayer(${player.id})">
                        <img src="${player.image_url || 'https://via.placeholder.com/150'}" alt="${player.nome} ${player.cognome}" class="player-image mx-auto mb-3">
                        <h3 class="font-semibold">${player.nome} ${player.cognome}</h3>
                        ${rating ? `
                            <div class="mt-2">
        ...
        try {
            ...
            container.innerHTML = players.map(player => `
                <div class="flex items-center justify-between p-3 border-b">
                    ...
                    <button onclick="deletePlayer(${player.id})" class="text-red-600 hover:text-red-800">
...
function ratePlayer(playerId) {
    if (window.app) {
        window.app.ratePlayer(playerId);
    }
}

function deletePlayer(playerId) {
    if (window.app) {
        window.app.deletePlayer(playerId);
    }
}
